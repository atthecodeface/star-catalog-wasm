...
`;
export function map_xyz_to_ra_de(xyz, ra_de) {
    return `
   // x = cos(de) * cos(ra)
   // y = cos(de) * sin(ra)
   // z = sin(de)
...
export class StarMapShader {
    constructor() {
        this.id = "star_map";
        this.extra_uniforms = ["magnitude"];
        this.vertex = `#version 300 es
  uniform mat4 projection;
  uniform mat4 view;
  uniform mat4 model;
...
export class StarShaderProjectedOntoNear {
    constructor() {
        this.id = "stars";
        this.extra_uniforms = ["magnitude"];
        this.vertex = `#version 300 es
