...
class FindOrientation {
    ...
    find_results(q_err) {
        ...
        for (let i=0; i<this.vectors.length; i++) {
            ...
            const angle = Math.acos(q.apply3(this.vectors[i]).dot(star.vector))*180/Math.PI;
    ...
    candidate_quality(c) {
        if (c >= this.candidates.length) {return null;}
        const candidate = this.candidates[c];
        ...
        for (let i=0; i<nvectors; i++) {
            for (let j=i+1; j<nvectors; j++) {
                qs.push(WasmQuatf64.mapping_vector_pair_to_vector_pair(this.vectors[i], this.vectors[j], stars[i].vector, stars[j].vector));
            }
        }
        while (qs.length > 1) {
            const merged_qs = [];
    ...
    find_triangles(i0, i1, i2) {
        ...
        return this.catalog.find_star_triangles(
            this.max_angle_delta, a12, a20, a01, 10000);
    }
    ...
    add_initial_triangles() {
        ...
        for (let i=0; i<n; i+=3) {
            ...
            const q01 = WasmQuatf64.mapping_vector_pair_to_vector_pair(vectors[0], vectors[1], s0.vector, s1.vector);
            const q12 = WasmQuatf64.mapping_vector_pair_to_vector_pair(vectors[1], vectors[2], s1.vector, s2.vector);
            const q20 = WasmQuatf64.mapping_vector_pair_to_vector_pair(vectors[2], vectors[0], s2.vector, s0.vector);
    ...
    add_further_triangle(i0, i1, i2) {
        ...
        for (let i=0; i<n; i+=3) {
            ...
            const s1 = this.catalog.star(candidates[i+1]);
            const s2 = this.catalog.star(candidates[i+2]);
            const q01 = WasmQuatf64.mapping_vector_pair_to_vector_pair(vectors[i0], vectors[i1], s0.vector, s1.vector);
            const q12 = WasmQuatf64.mapping_vector_pair_to_vector_pair(vectors[i1], vectors[i2], s1.vector, s2.vector);
            const q20 = WasmQuatf64.mapping_vector_pair_to_vector_pair(vectors[i2], vectors[i0], s2.vector, s0.vector);
            let d1 = q01.distance_sq(q12);
            let d2 = q12.distance_sq(q20);
            let d3 = q20.distance_sq(q01);
...
export class FindCanvas {
    ...
    constructor(star_catalog, catalog, canvas_div_id, width, height) {
        ...
        find_max_magnitude.addEventListener("input", this.set_parameters.bind(this));
        ...
        window.log.add_log(0, "project", "load", `Created find canvas`);
    }
    ...
    set_parameters(e) {
        ...
        if (m) {
            ...
            document.getElementById("find_mm").innerText = `Max magnitude ${this.max_magnitude}`;
        }
        const a = document.getElementById("find_max_angle");
        if (a) {
            this.max_angle_delta = a.value;
            document.getElementById("find_ma").innerText = `Max angle ${this.max_angle_delta}`;
        }
        const f = document.getElementById("find_fovh");
        if (f) {
            this.mm_equiv = f.value/1;
            const deg_fov = Math.atan(18/this.mm_equiv)*2*180/Math.PI;
            document.getElementById("find_fh").innerText = `${this.mm_equiv.toFixed(1)}mm (35mm eq) FOVH ${deg_fov.toFixed(1)}`;
        }
    }
    ...
    best_matches(e) {
        ...
        const find_orientation = new FindOrientation(this.catalog, star_vectors, max_magnitude, max_angle_delta);
        ...
        if (!q_err) {
            find_results.innerHTML = "Failed to find any matches - try adjusting the parameters";
        } else {
            find_results.innerHTML = find_orientation.find_results(q_err);
            this.star_catalog.sky_view_set_orientation(q_err[0].rotate_y(-Math.PI/2).rotate_x(Math.PI/2));
        }
    }
    ...
    vector_of_img_xy(ixy) {
        ...
        const sensor_yaw = Math.atan2(Math.sqrt(dx*dx+dy*dy), px_per_mm_div_x_lens_to_sensor_mm);
        const world_yaw = (1-0.031) * sensor_yaw + 0.3444*sensor_yaw*sensor_yaw*sensor_yaw + 0.2144*sensor_yaw*sensor_yaw*sensor_yaw*sensor_yaw*sensor_yaw;
        // const world_yaw = sensor_yaw;
        const world_dir = new WasmVec3f64(Math.cos(roll) * Math.tan(sensor_yaw),
    ...
    img_xy_of_vector(vector) {
        ...
        const sensor_yaw = (1+0.03212) * world_yaw + -0.393289*world_yaw*world_yaw*world_yaw + 0.2281345*world_yaw*world_yaw*world_yaw*world_yaw*world_yaw;
        // const sensor_yaw = world_yaw;
        const iy = Math.sin(sensor_yaw) * Math.sin(roll) * this.img_w + this.img_h/2;
        const ix = Math.sin(sensor_yaw) * Math.cos(roll) * this.img_w + this.img_w/2;
        return [ix, iy];
    }
    ...
    redraw_canvas() {
        ...
        for (const star of stars.stars) {
            ...
            const ixy = [ this.img_w/2 -v[1] / v[0] * this.img_w * this.mm_equiv/36,
    ...
    drag_to(xy) {
        console.log(xy);
        this.zoomed_window.zoom_scr_by(-xy[0]+this.drag_xy[0],
                                       -xy[1]+this.drag_xy[1],
                                      );
        this.drag_xy = xy;
        this.redraw_canvas();
