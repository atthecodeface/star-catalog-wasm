...
export class ViewProperties implements Application {
  ...
  map_mm_equiv_to_fovh(mm_equiv: number): number {
    const tan_hfovh = 18 / mm_equiv;
    return 2 * Math.atan(tan_hfovh);
  }

  map_fovh_to_zoom(fovh: number): number {
  ...
  map_zoom_to_fovh(zoom: number): number {
  ...
  derive_observer_frame() {
    ...
    //
    // The observed elevation is asin(z); the observed compass is atan2(y,x)
    this.view_observer_center_dir = this.ecef_to_observer_q.apply(
  ...
  update_html_elements() {
    ...
    if (e_zoom !== null) {
      (e_zoom as HTMLInputElement).value = this.map_fovh_to_zoom(
        this.fovh,
      ).toString();
    }
  ...
  time_add(hours: number, minutes: number, seconds: number) {
  ...
  view_observer_set_orientation(q: WasmQuatf64) {
  ...
  sky_view_center_on_ra_de(ra: number, de: number) {
